import './index.css';
import { Lokomotive, Train } from './train';

const train = new Train('train');
...
document.getElementById('undoBtn')!.addEventListener('click', () => train.removeTrain());
...
function buildPartGrid(parentId: string): void {
    ...
    for (const option of PART_OPTIONS) {
        ...
        button.addEventListener('click', () => train.addTrainpart(option.value));
